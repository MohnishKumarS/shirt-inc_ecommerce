                    ...
                    <div class="col-lg-6  border py-3 bg-white">
                        ...
                        <div class="mb-2">
                            <p class="text-sm">Estimated arrival on :
                                <span class="text-sm-bold" style="color: #212529"> {{ $est_date1 . ' - ' . $est_date2 }}
                                </span>
                            </p>
                        </div>
                        <div>
        ...
        @if (Session::has('total'))
            <div class="container text-center mx-auto">
                <form action="{{ url('/place-order') }}" method="POST" class="text-center mx-auto mt-5">
                    <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="rzp_test_gMw9oGZr3d5DEF"
                        data-amount="{{ Session::get('total') }}" data-currency="INR" data-order_id="{{ Session::get('order_id') }}"
                        {{-- data-buttontext="Pay with Razorpay" --}} data-name="Shirt-Inc" data-description="Thank you for choosing us...."
                        data-prefill.name='{{ Auth::user()->name }}' ; data-prefill.email='{{ Auth::user()->email }}' ;
                        data-prefill.contact='{{ Auth::user()->mobile }}' ; data-image="{{ asset('image/shirtinc-logo.png') }}"
                        data-theme.color="#3399cc" id="payment-order"></script>
